...
## Version 3.1.0 (2025/01/16) - 파일 미생성 문제 해결 및 정규식 개선

<details>
<summary>파일 미생성 문제 해결</summary>

- **파일 경로 중복 문제 해결**: 절대 경로와 상대 경로를 올바르게 처리하여 파일 경로 중복 오류 해결
- **정규식 매칭 개선**: `**파일명**` 마크다운 형식 지원으로 LLM 응답 파싱 성능 향상
- **파일 작업 안정성 향상**: 코드 블록과 삭제 파일 정규식 모두 개선하여 파일 생성/수정/삭제 성공률 대폭 향상
- **에러 처리 강화**: 파일 시스템 접근 오류에 대한 더 나은 에러 핸들링

</details>

<details>
<summary>기술적 개선사항</summary>

- **CodebaseContextService**: `path.isAbsolute()` 체크 추가로 파일 경로 처리 로직 개선
- **LlmResponseProcessor**: 정규식 패턴에 `(?:\*\*)?` 추가로 마크다운 강조 표시 지원
- **파일 경로 검증**: 절대 경로와 상대 경로를 구분하여 올바른 파일 경로 생성
- **정규식 최적화**: 더 정확한 파일 지시어 매칭으로 파싱 성능 향상

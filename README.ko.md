...
상세한 릴리즈 노트는 [RELEASE.ko.md](RELEASE.ko.md)를 참조하세요.

### 최신 버전 2.5.3 (2025/08/19) - 대화형 명령어 처리
...
- **동적 모델 선택**: 설정에서 AI 모델 드롭다운
- **자동 Bash 명령어 실행**: LLM 응답에서 터미널 명령어 실행
- **향상된 설정 인터페이스**: 개선된 설정 옵션

### Version 2.5.0 (2025/08/19) - Ollama 파일 작업 수정 및 정규식 지원 강화

- **파일 경로 파싱 수정**: Ollama 응답에서 파일명 `**` 접미사 문제 해결
- **정규식 패턴 강화**: 마크다운 헤더 처리 기능 추가
- **외부 서버 지원**: 외부 Ollama 서버 및 SSL 인증서 처리 개선
- **파일 작업 기능 강화**: 정확한 파일 매칭 및 다중 파일 지원

### Version 2.4.2 (2025/07/29) - 멀티모델 AI 지원 및 Ollama 통합

### Version 2.4.1 (2025/08/19) - LLM 프롬프트 구조 개선 및 코드 생성/수정 요청 방식 고도화

- LLM에게 엄격한 출력 형식과 규칙을 시스템 프롬프트로 명시
- 전체 파일 코드, 파일별 지시어, 작업 요약, 상세 설명 필수화
- AI의 작업 신뢰성 및 자동화 수준 향상

### Version 2.4.0 (2025/06/26) - AI 응답 구조 및 UX 개선

- 코드 생성/수정/삭제 작업 시 명확한 파일 작업 지시어와 전체 코드 출력 필수화
- 사용자 경험 개선: 메시지 표시 순서 최적화 및 이모지 인디케이터 추가
- 포괄적 국제화(i18n) 지원: 7개 언어 지원
- 실시간 정보 기능 강화: 날씨, 뉴스, 주식 정보

### Version 2.3b (2025/6/15) - 실시간 정보 기능 추가

- 날씨, 뉴스, 주식 정보 조회 기능 추가
- 외부 API 키 설정 옵션 추가
- 자연어 질의를 통한 실시간 정보 검색

### Version 2.2b (2025/06/10) - API 호환성 수정

- Gemini API 호환성 문제 수정
- API 호출 에러 처리 개선
...
**전체 릴리즈 히스토리는 [RELEASE.ko.md](RELEASE.ko.md)를 참조하세요**
